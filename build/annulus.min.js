...
var _annulus = _interopRequireDefault(__webpack_require__(/*! ./src/annulus/annulus.js */ "./src/annulus/annulus.js"));
...
var app = new _annulus.default.App({
  container_id: 'main'
});

/***/ }),

/***/ "./src/annulus/annulus.js":
/*!********************************!*\
  !*** ./src/annulus/annulus.js ***!
  \********************************/
/*! no static exports found */
...
const BACKGROUND = [50, 50, 50]; // Face 'enum'
...
function colorLerp(c1, c2, x) {
  return [Math.floor(c2[0] * x + c1[0] * (1.0 - x)), Math.floor(c2[1] * x + c1[1] * (1.0 - x)), Math.floor(c2[2] * x + c1[2] * (1.0 - x))];
}
...
class Vis {
  ...
  stressToColor(stress) {
    ...
    return scaled < .5 ? colorLerp(this._palette[0], this._palette[1], scaled * 2.0) : colorLerp(this._palette[1], this._palette[2], (scaled - .5) * 2.0);
  }
  ...
  makeFace(elem, r, color) {
    ...
    if (this.face == FACE_SIMPLE) {
      let y = .25 * r;
      let y2 = r * (.5 - this._parent._stress / EXCITEMENT_SCALE); // Mouth

      let p = " M " + -.5 * r + ", " + y + " Q0, " + (y2 + y) + ", " + .5 * r + ", " + y;
      makePath(elem, p, r, color); // Eyes
      ...
      let p = " M" + -.5 * r + ", " + y + " C" + -.25 * r + ", " + (y2 + y) + " " + .25 * r + ", " + (y1 + y) + " " + .5 * r + ", " + y;
      makePath(elem, p, r, color); // Eyes

      let x3 = r * (.2 * this._parent._stress / EXCITEMENT_SCALE);
      let y3 = r * (.12 * this._parent._stress / EXCITEMENT_SCALE);
      let eyes = " M" + -.6 * r + ", " + 0 + " l" + -x3 + ", " + -y3 + " M" + -.6 * r + ", " + 0 + " l" + -x3 + ", " + y3 + " M" + .6 * r + ", " + 0 + " l" + x3 + ", " + -y3 + " M" + .6 * r + ", " + 0 + " l" + x3 + ", " + y3;
      ...
      let p = " M" + -dx0 + ", " + y // P0 
      + " C" + (-dx0 + dx2) + ", " + (y - dy2) + " " + -dx4 + ", " + (y + dy3) + " " + -dx1 + ", " + (y + dy1) // P1
      + " m0,0" + " C" + -dx3 + ", " + (y + dy4) + " " + dx3 + ", " + (y + dy4) + " " + dx1 + ", " + (y + dy1) // P2
      + " m0,0" + " C" + dx4 + ", " + (y + dy3) + " " + (dx0 - dx2) + ", " + (y - dy2) + " " + dx0 + ", " + y; // P3
      ...
      dy2 = r * (.1 + 0 * s2);
      dy3 = r * (.1 - .2 * s2);
      let eyes = " M" + -dx1 + ", " + dy1 + " l" + -dx3 + ", " + dy3 + " M" + -dx1 + ", " + dy1 + " l" + -dx2 + ", " + dy2 + " M" + dx1 + ", " + dy1 + " l" + dx3 + ", " + dy3 + " M" + dx1 + ", " + dy1 + " l" + dx2 + ", " + dy2;
      makePath(elem, eyes, r, color);
    }
  }
...
class RingVis extends Vis {
  ...
  update(elem) {
    ...
    const color_str = rgbToHex(col); // Clear out the old stuff

    while (elem.lastChild) elem.removeChild(elem.lastChild);

    while (this._parent.debugg.lastChild) this._parent.debugg.removeChild(this._parent.debugg.lastChild); // Step one, the central path (cicle/cloud shape)
    ...
    for (let i = 0; i < out_segs; i++) {
      ...
      if (i == out_segs - 1) theta = Math.PI * 2;else theta += theta_step * (1 - osc + .5);
      ...
      p += " C" + cx1.toFixed(5) + "," + cy1.toFixed(5) + " " + cx2.toFixed(5) + "," + cy2.toFixed(5) + " " + px.toFixed(5) + "," + py.toFixed(5);
      ...
      if (this._parent._showDebug) {
        ...
        l1.setAttributeNS(null, 'd', "M" + cx2.toFixed(5) + "," + cy2.toFixed(5) + " L" + px.toFixed(5) + "," + py.toFixed(5) + " M" + cx1.toFixed(5) + "," + cy1.toFixed(5) + " L" + prev_px.toFixed(5) + "," + prev_py + " Z");
    ...
    circle.setAttributeNS(null, 'd', p); // The face
    //
...
class WeerVis extends Vis {
  ...
  update(elem) {
    ...
    const color_str = rgbToHex(super.stressToColor(this._parent.stress)); // Clear out old stuff

    while (this._parent.debugg.lastChild) this._parent.debugg.removeChild(this._parent.debugg.lastChild);

    while (elem.lastChild) elem.removeChild(elem.lastChild); // Step one, the central path (cicle/cloud shape)
    ...
    for (let i = 0; i < out_segs; i++) {
      ...
      let prev_theta = theta;
      if (i == out_segs - 1) theta = Math.PI * 2;else theta += theta_step * .75 + theta_step * .25 * (1 - osc + .45); //theta +=theta_step;
      //theta =next_theta();
    ...
    circle.setAttributeNS(null, 'd', p); // Step two. The face
    //
    ...
    let line_segs = 2 * in_segs + in_segs * 2 * (1 - this._parent._stress); //let skip_lines =Math.floor(in_segs * 2 * (1.1- this._parent._stress)); 
    ...
    for (let i = 0; i < line_segs; i++) {
      ...
      let prev_theta = theta;
      if (i == line_segs - 1) theta = 0; //Math.PI/4 * s - theta_step;
      else theta += theta_step * .75 + theta_step * .25 * (1 - osc + .45); //if( i % skip_lines == 0 ) continue;

      hdist = 4 / 3 * Math.tan((theta - prev_theta) / 4); // Points of the middle/tangential line

      let px1 = Math.cos(prev_theta) * re;
      let py1 = Math.sin(prev_theta) * re;
      let px2 = Math.cos(theta - .75 * theta_step) * re;
      let py2 = Math.sin(theta - .75 * theta_step) * re; // Outer/inner points that form the 'spark'     
      ...
      e.setAttributeNS(null, 'stroke', color_str
      /*rgbToHex(this.color0)*/
      );
...
class Annulus {
  constructor(svgarea) {
    ...
    svgarea.viewbox = [-2, -2, 4, 4]; // Add a group for visualisation elements

    this.pathg = document.createElementNS('http://www.w3.org/2000/svg', 'g');
    this.svgarea.svg_elem.appendChild(this.pathg); // Add another group for debug elements
  ...
  get animateStress() {
    return this._animateStress;
  }

  set animateStress(b) {
    this._animateStress = b;
  }

  get segments() {
    return this._vis.segments;
  }

  set segments(n) {
    this._vis.segments = n;
    this.update();
  }

  get thickness() {
    return this._vis.thickness;
  }

  set thickness(n) {
    this._vis.thickness = n;
    this.update();
  }
...
class App {
  constructor({
    ...
    console.log('niemand ter aarde weet, hoe het eigenlijk begon, het droevige verhaal van de nozem en de non'); // Create a viewport container inside the given element id

    this.container_elem = document.getElementById(container_id);
    if (typeof this.container_elem == 'undefined') this.container_elem = document.body;
    let svg_elem = document.createElement('object');
    svg_elem.className = 'svgarea';
    this.container_elem.appendChild(svg_elem);
    this.svgarea = new SVGArea({
      container_elem: svg_elem
    });
    this.annulus = new Annulus(this.svgarea);
    this.visList = {
      "Ring": new RingVis(this.annulus),
      "Weer": new WeerVis(this.annulus)
    };
    this._vis = "";
    this.vis = "Ring";
  ...
  setupToolbox() {
    ...
    f_a.add(this.annulus, 'face', {
      None: FACE_NONE,
      Simple: FACE_SIMPLE,
      Troubled: FACE_TROUBLED,
      Sad: FACE_SAD,
      Complex: FACE_COMPLEX
    }).name('Face');
    f_a.open(); // Colors toolbox
    ...
    f_p.add(this.annulus, 'showDebug').name('Debug'); //f_p.add( this.annulus, 'frame', 0.0, 1.0 ).name( 'Frame' ).step( 0.01);
...
/******/ });
//# sourceMappingURL=annulus.min.js.map