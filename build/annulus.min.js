...
class Annulus {
  constructor(svgarea) {
    ...
    svgarea.viewbox = [-2, -2, 4, 4]; // Create an SVG node for the actual visualisation

    this._path = document.createElementNS('http://www.w3.org/2000/svg', 'path');

    this._path.setAttributeNS(null, 'fill', 'blue');

    this.svgarea.svg_elem.appendChild(this._path); // Add another group for debug elements
  ...
  stressToColor(stress) {
    ...
    return scaled < .5 ? this.colorLerp(this._palette[0], this._palette[1], scaled * 2.0) : this.colorLerp(this._palette[1], this._palette[2], (scaled - .5) * 2.0);
  }
  ...
  createPath() {
    ...
    const s = Math.pow(this._stress, 2) + 0.05; // Clear out the debug stuff
    ...
    let hdist = 0; // TODO: either use or delete
    ...
    let next_theta = function () {
      ...
      prng ^= prng >>> 7;
      prng ^= prng << 17;
      let q = prng / (-1 >>> 0) + 1.0; //console.log( q );
    ...
    for (let i = 0; i <= out_segs; i++) {
      ...
      theta += theta_step * (1 - osc + .49); //theta =next_theta();
...
class App {
  ...
  setupToolbox() {
    ...
    f_c.addColor(this.annulus, 'color0').name('Low').onFinishChange(() => {
      this.annulus.update();
    });
    f_c.addColor(this.annulus, 'color1').name('Medium');
    f_c.addColor(this.annulus, 'color2').name('High'); //        f_c.addColor( this.viewport, 'backgroundColor' ).name( 'Background' );
